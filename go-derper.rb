...
#    end
     ...
     #cachedump seems to be supported without the need for remote debug
#TODO: add a version check here... possibly not supported on all versions
     return true
  end
  ...
  when :fingerprintfile
    File.open(mode_inputs).readlines.join.split(/\n/).each do |server|
      begin
        ...
        puts "Exception in fingerprint mode: #{e.to_s}"
      end
    end
  when :leach
